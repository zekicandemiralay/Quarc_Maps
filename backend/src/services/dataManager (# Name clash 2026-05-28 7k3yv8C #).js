...
async function findFeedsForLocation(lat, lng, countryCode, countryMode = false) {
  const seen    = new Set()
  const results = []
  ...
  // In country mode use the full country bbox for Transitland so we get all regional feeds
  const tlBbox   = countryMode ? (COUNTRY_BY_CC[countryCode]?.bbox ?? null) : null
  const tlAdded  = addUnique(await findFeedsTransitland(lat, lng, tlBbox), 'transitland')
  const mdbAdded = addUnique(await findFeedsMDB(lat, lng, countryCode, countryMode), 'mobilitydata')
...
async function runDownload(k, lat, lng) {
  ...
  const countryMode = isCountryKey(k)
  ...
  console.log(`[dataManager] ── Starting download for region ${k} (${lat}, ${lng}) port ${port} countryMode=${countryMode} ──`)

  try {
    // For country-mode keys we already know the country; skip the reverse-geocode call.
    const countryCode = countryMode ? k : await getCountryCode(lat, lng)
    ...
    const tz = COUNTRY_BY_CC[countryCode]?.tz ?? getTimezoneForLocation(lat, lng, countryCode)
    console.log(`[dataManager] Timezone: ${tz}`)

    // ── 1. Find feeds ─────────────────────────────────────────────────────
    upd({ message: countryMode ? `Searching national transit catalog (${countryCode})…` : 'Searching global transit catalog…' })
    const feeds = await findFeedsForLocation(lat, lng, countryCode, countryMode)
    ...
    const MAX_ATTEMPTS = countryMode ? 100 : 30
    const MAX_VALID    = countryMode ?  50 :  3   // collect all national feeds in country mode
    const MAX_SIZE_MB  = countryMode ? 600 : 150  // national feeds can be large
    ...
    for (const feed of sorted) {
      ...
      // In country mode skip geographic check — we want all national feeds regardless of
      // where within the country the user's request originated.
      const v = validateGtfs(dest, tz, countryMode ? null : lat, countryMode ? null : lng)
      if (!v.valid) {
    ...
    await downloadOSM(lat, lng, countryCode, upd, rDir)
...
async function downloadOSM(lat, lng, countryCode, upd, destDir) {
